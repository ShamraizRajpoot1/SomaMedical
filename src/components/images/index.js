...
export const SqareRound = ({ style, size, source, height, width }) => {
    const defaultSize = totalSize(5)
    return (
        <Image
            source={source}
            
            style={[{ height: height ? height : size ? size : defaultSize, width: width ? width : size ? size : defaultSize, borderRadius: 15 }, style]}
        />
    );
}
